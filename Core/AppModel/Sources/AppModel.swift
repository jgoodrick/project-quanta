 ...
 */
public struct AppModel: Sendable, Equatable {
...
extension SharedReaderKey where Self == InMemoryKey<AppModel>.Default {
    static public var model: Self {
        Self[.inMemory("AppModel"), default: .init()]
    }
}

extension SharedReaderKey where Self == FileStorageKey<Database>.Default {
...
extension SharedReaderKey where Self == InMemoryKey<AppModel.Config>.Default {
    fileprivate static var config: Self {
        Self[
            .inMemory("AppModel.Config"),
            default: .init()
        ]
    }
}
