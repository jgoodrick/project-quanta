...
struct EntryRow: View {
    ...
    private var subtitleText: Text {
        let tagsText = tags.reduce(Text(entry.date == nil ? "" : "  ")) { result, tag in
            result + Text("#\(tag) ")
                .foregroundStyle(.gray)
                .bold()
        }
    ...
    private func title(for entry: Entry) -> some View {
        let exclamations = String(repeating: "!", count: entry.priority ?? 0)
        + (entry.priority == nil ? "" : " ")
        return (
            Text(exclamations)
                .foregroundStyle(entry.isCompleted ? .gray : Color.hex(entryCollection.color))
            + Text(entry.title)
                .foregroundStyle(entry.isCompleted ? .gray : .primary)
        )
        .font(.title3)
    }
}
