    ...
    var body: some View {
        ...
        ForEach(searchEntries.entries, id: \.entry.id) { entry in
            EntryRow(
                ...
                entry: entry.entry,
                entryCollection: entry.entryCollection,
    ...
    struct SearchEntries: FetchKeyRequest {
        ...
        func fetch(_ db: Database) throws -> Value {
          ...
          """)
            ...
            //     "entryCollections" into the above query and decode it into 'State'. Ideally this
            //     could all be done with a single query.
            let entryCollections = try EntryCollection.fetchAll(
                db,
                keys: Set(entries.map(\.entryCollectionID))
            )
            ...
            return Value(
                ...
                entries: entries.map { entry in
                    Value.Entry(
                        isPastDue: entry.isPastDue,
                        entry: entry.entry,
                        entryCollection: entryCollections.first(where: { $0.id == entry.entry.listID} )!,
