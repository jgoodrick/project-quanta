...
public struct EntryDetail {
    ...
    public struct State: Equatable {
        ...
        public init(entry entryID: Entry.ID, translationsEditorFocused: Bool) {
            let shared = Shared(value: entryID)
    ...
    public var body: some ReducerOf<Self> {
        ...
        Reduce<State, Action> { state, action in
            ...
            case .addLanguageMenuButtonTapped(let language):
                ...
                state.$model.withLock({ $0.addExisting(language: language.id, toEntry: state.entryID) })
            ...
            case .languageCellDestructiveSwipeButtonTapped(let language):
                ...
                _ = state.$model.withLock({ $0.remove(language: language.id, fromEntry: state.entryID) })
            ...
            case .translationCellDestructiveSwipeButtonTapped(let translation):
                ...
                if state.model.entity(.entry(translation.id), wouldBeOrphanIfRemovedFrom: .entry(state.entryID)) {
                    ...
                    _ = state.$model.withLock({ $0.remove(translation: translation.id, fromEntry: state.entryID) })
                }
            ...
            case .usageCellDestructiveSwipeButtonTapped(let usage):
                ...
                if state.model.entity(.usage(usage.id), wouldBeOrphanIfRemovedFrom: .entry(state.entryID)) {
                    ...
                    _ = state.$model.withLock({ $0.remove(usage: usage.id, fromEntry: state.entryID) })
                }
            ...
            case .noteCellDestructiveSwipeButtonTapped(let note):
                ...
                if state.model.entity(.note(note.id), wouldBeOrphanIfRemovedFrom: .entry(state.entryID)) {
                    ...
                    _ = state.$model.withLock({ $0.remove(note: note.id, fromEntry: state.entryID) })
                }
            ...
            case .movedLanguage(fromOffsets: let fromOffsets, toOffset: let toOffset):
                ...
                state.$model.withLock({ $0.moveLanguages(onEntry: state.entryID, fromOffsets: fromOffsets, toOffset: toOffset) })

                return .none
                
            case .movedTranslation(fromOffsets: let fromOffsets, toOffset: let toOffset):
                
                state.$model.withLock({ $0.moveTranslations(on: state.entryID, fromOffsets: fromOffsets, toOffset: toOffset) })

                return .none
                
            case .movedUsage(fromOffsets: let fromOffsets, toOffset: let toOffset):
                
                state.$model.withLock({ $0.moveUsages(on: state.entryID, fromOffsets: fromOffsets, toOffset: toOffset) })

                return .none
                
            case .movedNote(fromOffsets: let fromOffsets, toOffset: let toOffset):
                
                state.$model.withLock({ $0.moveNotes(on: state.entryID, fromOffsets: fromOffsets, toOffset: toOffset) })
...
extension Language: @retroactive CategorizedItem {
...
extension Entry: @retroactive CategorizedItem {
    public var value: String { spelling }
}

extension Usage: @retroactive CategorizedItem {}
extension Language: @retroactive CategorizedItemsSectionCategory {
...
extension Note: @retroactive TextEditableItem {}
