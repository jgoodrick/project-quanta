...
/*
 Credit: https://github.com/rudrankriyam/Quotex
 */
...
private struct Tag: Hashable, Identifiable {
    var id: String { title }
    var title: String
}
...
private struct Preview: View {
    ...
    var body: some View {
        Color.clear.overlay(alignment: .leading) {
            TagLayout(alignment: .leading, spacing: 0) {
                ForEach(tags) { tag in
                    Text(tag.title)
                        .padding()
...
#Preview {
    Preview(tags: [.init(title: "tags here"), .init(title: "because"), .init(title: "this preview should show how"), .init(title: "it will"), .init(title: "look")])
}
