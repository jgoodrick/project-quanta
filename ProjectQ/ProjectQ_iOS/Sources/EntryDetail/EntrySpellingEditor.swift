...
//            }
//
//            if let match = model.firstEntry(where: { $0.spelling == text }) {
//
...
//            }
//
//            guard var value = model[entry: entryID] else {
//                assertionFailure()
//                return
//            }
//            
...
//            case .editSpellingButtonTapped:
//
//                state.textField.text = state.model[entry: state.entryID]?.spelling ?? ""
...
//                
//                state.model.merge(entry: preMergeID, into: existing.id)
//
//                return .none
//
...
//    static func addOrMergeWithExisting(entry: Entry) -> Self where Action == EntrySpellingEditor.ConfirmationDialog {
//        .init(
//            title: {
