...
extension EntryDetail.State {
    ...
    mutating func submitText() -> EffectOf<EntryDetail> {
        ...
        switch textField.target {
        case .spelling:
            switch $model.withLock({ $0.attemptToUpdateEntrySpelling(of: entryID, to: textField.text) }) {
        ...
        case .newTranslation:
            switch $model.withLock({ $0.attemptToAddNewTranslation(fromSpelling: textField.text, forEntry: entryID) }) {
            case .success, .canceled: break
            case .conflicts(let conflicts):
        ...
        case .usage(let id):
            ...
            if let id {
                $model.withLock({ $0.updateUsage(\.value, of: id, to: textField.text) })
            } else if !textField.text.isEmpty {
                _ = $model.withLock({ $0.attemptToAddNewUsage(content: textField.text, toEntry: entryID, autoAppliedValueConflictResolution: .mergeWithFirstMatch) })
            }
            
        case .note(let id):
            if let id {
                $model.withLock({ $0.updateNote(\.value, of: id, to: textField.text) })
            } else if !textField.text.isEmpty {
                _ = $model.withLock({ $0.attemptToAddNewNote(content: textField.text, toEntry: entryID) })
            }
    ...
    mutating func resolveSpellingUpdateConflict(resolution: EntryDetail.SpellingConflictResolution) {
        ...
        let result = $model.withLock({
            $0.attemptToUpdateEntrySpelling(
                of: entryID,
    ...
    mutating func resolveOrphanedIfRemoved(resolution: EntryDetail.OrphanedIfRemovedResolution) {
        ...
        case .cancel: break
        case .delete: $model.withLock({ $0.delete(resolution.target.entity.id) })
        case .disconnectOnly:
